...
class EnhancedBuilder {
  ...
  async applyPerformanceOptimizations() {
    ...
    for (const file of htmlFiles) {
      ...
      this.log(
        `Created optimized version: ${path.basename(optimizedPath)}`,
        'success'
      );
    }
  ...
  async generateSecurityReport() {
    ...
    try {
      ...
      report.checks.push({
        name: 'NPM Audit',
        status:
          auditData.metadata.vulnerabilities.total === 0 ? 'PASS' : 'FAIL',
    ...
    const hasStrictCSP =
      !nginxContent.includes('unsafe-inline') ||
    ...
    this.log(
      `Security Report: ${passed} passed, ${failed} failed, ${errors} errors`,
  ...
  async generatePerformanceReport() {
    ...
    // Check file sizes
    const distFiles = ['dist/css/styles.min.css', 'dist/js/main.min.js'];
